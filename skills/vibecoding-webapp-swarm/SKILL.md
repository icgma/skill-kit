...
description: 全栈多智能体 Web 应用协同开发流水线。支持丰富交互、多页面路由与全栈后端结合的应用级系统开发。
category: 工程研发
tags: [Web应用, 全栈开发, 前端构建, React应用]
---
...
7. **Create the version once per delivery and STOP.** One `mshtools-website_version_manager` `build_version` call per delivery, then no verification loop: do NOT open the URL, take screenshots, or review. A `build_version` call is forbidden until the final merged source has been freshly installed and `npm run build` has succeeded in the delivery worktree (see Phase 7 step 4). Each delivery merges its branch back into master; later fixes and rollbacks fork fresh branches from master (see "Iterating After Delivery").
...
4. **Install dependencies and build before versioning** (after all branches are merged and routes/contracts are wired):
   ```bash
   cd $HOME/app-final-build &&
     npm install --prefer-offline --no-audit &&
     npm run build 2>&1
   ```
   `setup-local.sh` installs before later branches are merged, so this final install reconciles subagent dependency changes. If install or build fails, fix the final worktree and rerun both commands (up to 3 retries). Do not call `build_version` until the build exits 0; any later source or dependency change invalidates this gate.
...
6. **Create the version — the deploy path depends on `type`.** `mshtools-website_version_manager` (`build_version`) records the source in `project_dir`; **where `project_dir` must point differs by type**, because static and dynamic previews are served differently. Do NOT copy `dist/` around by hand.

   **Full-stack → `type: "dynamic"`** (backend graft present; requires `dynamic` — `static` would serve the frontend only, without tRPC/OAuth/DB). Dynamic is **built server-side and started as a dev server from the worktree** on preview, so the deploy source is the worktree (which carries `node_modules` + `.env`):
   - `action: "build_version"`, `type: "dynamic"`, `project_dir: $HOME/app-final-build`, `message`: concise title.
   - Then **merge the delivery branch into master** so master contains the version (later fixes/rollbacks fork from it):
     ```bash
     cd /mnt/agents/output/app && git merge final-build --no-edit
     ```

   **Frontend-only → `type: "static"`** (after the mandatory install + `npm run build` gate). The static version is served from `/mnt/agents/output/app` (the platform builds the committed source there), so for static, `project_dir` **MUST be `/mnt/agents/output/app`**. Merge the final source into master there, then version it — the merge **is** the delivery, so there is no separate merge-back:
   ```bash
   cd /mnt/agents/output/app && git merge final-build --no-edit
   ```
   - Then `action: "build_version"`, `type: "static"`, `project_dir: /mnt/agents/output/app`, `message`: concise title.

   Follow the tool's own schema for anything not listed here. If the tool returns a URL, present it; **if it returns only a version ID and no URL, tell the user the version was saved with that ID** — do not fabricate a URL.

7. **STOP.** No verification after creating the version.
...
3. **Main agent**: Build in a worktree, then deliver via the **static** path (Mode B is always frontend-only):
   ```bash
   cd /mnt/agents/output/app && git branch final
   ...
   cd $HOME/app-final &&
     ...
     (git diff --cached --quiet || git commit -m "final: verify build")
   ```
   The static version is served from `/mnt/agents/output/app`, so merge into master there and version it (the merge **is** the delivery — no separate merge-back):
   ```bash
   cd /mnt/agents/output/app && git merge final --no-edit
   ```
   `action: "build_version"`, `type: "static"`, `project_dir: /mnt/agents/output/app`, `message`: concise title. **No verification after.** Later fixes and rollbacks follow "Iterating After Delivery".
...
`master` on the shared repo is the only durable lineage; branches and worktrees are per-operation and disposable. Every post-delivery operation — a fix, a rollback — **forks a fresh branch from master and builds in a fresh worktree**, then delivers **by the same `type` branch as Phase 7 step 6**: *dynamic* points the version tool at the worktree and merges the branch back into master; *static* merges the branch into master at `/mnt/agents/output/app` and points the tool there (the merge **is** the delivery — no separate merge-back). Do not reuse an earlier operation's worktree or branch: a reused worktree is stale relative to master once anything else merged, it can fold leftover debris into the version tool's commits, and one branch supports exactly one worktree.

The version tool is git-native: `build_version` commits the state of `project_dir` (the version ID is that commit's short hash); `rollback` restores the tree from a version commit and commits the restored state (roll forward, no history rewrite). For **dynamic** the commit lands on the worktree branch and the merge-back keeps master equal to the delivered version; for **static** the version is recorded directly on `/mnt/agents/output/app`. Worktrees share the shared repo's object store, so any version ID resolves in any fresh worktree.
...
Re-run Phase 7's contract checks and the mandatory install + build gate (steps 3-4) in this worktree, then deliver **by the same `type` branch as Phase 7 step 6**:

```bash
# dynamic: version the worktree, then merge back
#   build_version  project_dir=$HOME/app-fix-<n>  type=dynamic
cd /mnt/agents/output/app && git merge fix-<n> --no-edit

# static: merge into master, then version it there (the merge is the delivery)
cd /mnt/agents/output/app && git merge fix-<n> --no-edit
#   build_version  project_dir=/mnt/agents/output/app  type=static
```
...
```bash
# dynamic: fork a worktree, roll back there, then merge back
...
# tool: action="rollback", version_id="<id>", project_dir="$HOME/app-rollback-<version-id>"
...
# static: roll back directly on the coordination repo (its deploy path) — no worktree, no merge-back
# tool: action="rollback", version_id="<id>", project_dir="/mnt/agents/output/app"
```

- **`project_dir` follows the `type` rule (same as Phase 7 step 6):** a **dynamic** version is built server-side / run as a dev server from its worktree, so that worktree must carry `node_modules` + `.env` — never point a *dynamic* version at `/mnt/agents/output/app` (no deps there). A **static** version must point at `/mnt/agents/output/app` — its deploy folder is keyed to that path.
- Every operation still builds in a fresh worktree (fast disk); only the final `project_dir` and merge-back differ by type — so a follow-up forward fix just continues from the same worktree.
...
Websites and webapps you build here run on the Kimi platform (kimi.com). Before you deliver (save the version) or tell the user how to reach their site, read `product-knowledge.md` in this skill's directory: how preview and publish work (they are different — the tool returns a version ID, not a URL, and the version card is the preview), what the platform can actually do (full-stack DB, Kimi login, public publishing, versioning, custom URL, export), how data persists, and common user issues with the right response. Do not guess your environment's capabilities — read it.

Read: `vibecoding-webapp-swarm/product-knowledge.md`
