...
> **范围限制**：`bond_data` 所有工具均为**单只债券**查询（指定某一只债券的档案、行情、估值或发债主体财务），不支持全市场/跨券种的排名、筛选、Top-N 列表。若用户问题含"排名"、"最活跃"、"前 N"、"成交额最大"、"筛选"等跨债券集合意图，直接走 `analytics_data.get_financial_data`，不要尝试 `bond_data`。
