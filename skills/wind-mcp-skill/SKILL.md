---
name: wind-mcp-skill
description: Wind 金融终端数据网关（MCP）。提供 A 股/港美股/基金/债券/宏观经济指标与财报官方数据提取接口。
category: 商业金融
tags: [Wind数据, 金融终端, 行情提取, 财报数据, 宏观指标]
---
...
| `bond_data` | 指定单只债券的档案、发债主体、行情估值、主体财务；不支持跨券种排名/筛选/Top-N | `references/bond.md` |
...
4. 未指定具体标的的筛选/排名请求 → 对应领域的 `search_*`（股票/基金/指数有专项筛选工具）；债券没有跨券种筛选工具，债券排名/最活跃/Top-N 直接走 `analytics_data`。`analytics_data` 返回计算结果，不返回实体列表。
...
> 注意：当 Wind 上游返回空数据时，CLI 可能返回 `EMPTY_DATA` / `NO_RESULTS` 类回执，这不属于网络或后端错误，**不得按 `NETWORK_ERROR` 重试**，应直接报告无结果并结束。若后端 message / payload snippet 出现"没找到数据"、仅返回市场聚合值而无个券列表、或银行间与交易所两个市场均无个券数据，也视为空结果，**同一意图最多改写一次**后仍空则直接报 `NO_RESULTS` 并给出替代方案（改查最近完整交易日、指定券种或单只债券）。
