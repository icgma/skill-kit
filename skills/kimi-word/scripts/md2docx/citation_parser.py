...
# Horizontal whitespace only — \s* would swallow \n\n and merge paragraphs.
...
def _preserve_non_numeric_markers(markdown_content: str) -> str:
    """Escape unresolved markers so Pandoc renders their original visible text.

    This intentionally changes Markdown parsing semantics: non-numeric footnote
    definitions are rendered as literal marker text rather than Pandoc footnotes.
    """
    markdown_content = _RE_NON_NUMERIC_CARET.sub(
        lambda m: rf'\[\^{m.group(1)}\^\]', markdown_content
    )
    return _RE_NON_NUMERIC_BRACKET.sub(
        lambda m: rf'\[\^{m.group(1)}\]', markdown_content
    )
...
def convert_citations(markdown_content: str, citation_db: Dict[int, dict]) -> tuple:
    ...
    id_to_display_num = {}
    pandoc_safe_content = _preserve_non_numeric_markers(markdown_content)

    if not detection['converted']:
        return pandoc_safe_content, id_to_display_num, detection
    ...
    def _missing_marker(cid: int) -> str:
        if fmt == 'caret':
            return rf'\[\^{cid}\^\]'
        if fmt == 'bracket_caret':
            return rf'\[\^{cid}\]'
        return rf'\[{cid}\]'

    def _replace_cluster(match):
        ids = re.findall(extract_pattern, match.group(0))
        trailing_whitespace = re.search(r'\s*$', match.group(0)).group(0)
        replacements = []
        for id_str in ids:
            cid = int(id_str)
            if cid in id_to_display_num:
                replacements.append(f'^{id_to_display_num[cid]}^')
            else:
                missing_ids.add(cid)
                # Preserve unresolved source IDs so conversion never destroys
                # evidence pointers. The warning below lets the caller decide
                # whether to repair the citation database or keep the marker.
                replacements.append(_missing_marker(cid))
        return ' '.join(replacements) + trailing_whitespace

    converted = cluster_regex.sub(_replace_cluster, pandoc_safe_content)

    if missing_ids:
        logger.warning(f"Citation IDs not in citation_db, preserved in body: {sorted(missing_ids)}")
