...
def _add_bookmark(p_element, bookmark_id, bookmark_name):
    """Wrap a paragraph in a bookmark. CT_P requires pPr as the first child,
    so bookmarkStart goes after it, never at raw position 0."""
...
def process_word_document_with_hyperlinks(docx_path: str, output_path: str = None, citation_db: dict = None) -> bool:
    ...
    try:
        ...
        # Allocate bookmark ids above anything already in the document —
        # pandoc's heading bookmarks start low and collide with a fresh 1..N range.
        existing_bk_ids = [
            int(b.get(qn('w:id')))
            for b in doc.element.body.iter(qn('w:bookmarkStart'))
            if (b.get(qn('w:id')) or '').lstrip('-').isdigit()
        ]
        bookmark_id = max(existing_bk_ids, default=0) + 1
        ...
        for para in doc.paragraphs:
            ...
            if in_references:
                ...
                if match:
                    ...
                    _add_bookmark(p_element, bookmark_id, bookmark_name)
                    bookmark_id += 1
        ...
        if not has_references and citation_db:
            ...
            for num in sorted(citation_db.keys()):
                ...
                _add_bookmark(ref_para._element, bookmark_id, f'Note_{num}')
                bookmark_id += 1
        ...
        for para in doc.paragraphs:
            ...
            for run in para.runs:
                if run.font.superscript and run.text.strip():
                    text = run.text.strip()
                    if re.match(r'^\d+$', text) and int(text) in citation_db:
