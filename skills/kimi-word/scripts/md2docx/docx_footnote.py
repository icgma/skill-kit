...
def process_word_document_with_noteref(
    ...
    try:
        ...
        valid_citation_numbers = set((citation_db or {}).keys())
        ...
        for run in root.iter(qn('w:r')):
            ...
            if vert_align is not None and vert_align.get(qn('w:val')) == 'superscript':
                ...
                if text_elem is not None and text_elem.text:
                    ...
                    if re.match(r'^\d+$', text) and int(text) in valid_citation_numbers:
                        num = int(text)
                        para = run.getparent()
        ...
        # Word numbers footnotes by document order of their reference marks, so
        # pandoc's own footnotes take slots too. Compute each citation footnote's
        # real ordinal — NOTEREF cached text must match what Word shows without a
        # field refresh (bookmark names keep the display numbering).
        first_run_by_id = {id(run): num for num, (para, run) in citation_first_occurrence.items()}
        actual_num = {}
        seq = 0
        for elem in root.iter():
            if elem.tag == qn('w:footnoteReference'):
                seq += 1
            elif elem.tag == qn('w:r') and id(elem) in first_run_by_id:
                seq += 1
                actual_num[first_run_by_id[id(elem)]] = seq

        # footnotes.xml: base.docx may already contain pandoc footnotes (standard
        # [^N] md syntax) — merge into the existing part with fresh ids; overwriting
        # it would leave every existing footnoteReference dangling.
        footnotes_xml_path = temp_path / 'word' / 'footnotes.xml'
        if footnotes_xml_path.exists():
            footnotes_root = etree.parse(str(footnotes_xml_path)).getroot()
            existing_ids = [
                int(fn.get(qn('w:id')))
                for fn in footnotes_root.findall('w:footnote', nsmap)
                if int(fn.get(qn('w:id'))) > 0
            ]
            next_id = max(existing_ids, default=0) + 1
        else:
            footnotes_root = _create_footnotes_shell(nsmap)
            next_id = 1
        # Display number (bookmarks / NOTEREF) is decoupled from footnote w:id.
        id_map = {num: next_id + i for i, num in enumerate(sorted(citation_first_occurrence))}
        _append_footnotes(footnotes_root, id_map, citation_db or {}, nsmap)
        etree.ElementTree(footnotes_root).write(
            str(footnotes_xml_path),
            encoding='utf-8', xml_declaration=True, standalone=True
        )
        logger.info(f"Wrote footnotes.xml with {len(citation_first_occurrence)} citation footnotes")

        update_document_rels(temp_path, nsmap, 'footnote')
        ...
        for num, occurrences in citation_all_occurrences.items():
            for idx, (para, run) in enumerate(occurrences):
                ...
                if idx == 0:
                    parent.replace(run, _footnote_ref_run(id_map[num], nsmap))
                    footnote_count += 1
                else:
                    parent.replace(run, _noteref_field(num, nsmap, display=actual_num.get(num, num)))
                    noteref_count += 1
        ...
        update_content_types(temp_path, nsmap, 'footnote')

        with ZipFile(output_path, 'w') as zout:
            for fp in temp_path.rglob('*'):
                if fp.is_file():
                    zout.write(fp, fp.relative_to(temp_path).as_posix())
...
def _create_footnotes_shell(nsmap):
    """Fresh footnotes.xml root with the two separator footnotes Word requires."""
...
def _append_footnotes(footnotes, id_map, citation_db: dict, nsmap):
    """Append citation footnotes with bookmarks; content from citation_db.

    id_map maps display number -> footnote w:id, so appended notes never
    collide with footnotes already present in the part.
    """
    for num in sorted(id_map):
        footnote = etree.SubElement(footnotes, qn('w:footnote'))
        footnote.set(qn('w:id'), str(id_map[num]))
...
def _noteref_field(num, nsmap, display=None):
    """Subsequent occurrences: create NOTEREF field (\\h \\f switches).

    The instruction references the bookmark by display number; the cached
    text must be Word's real ordinal (document order of reference marks).
    """
    if display is None:
        display = num
