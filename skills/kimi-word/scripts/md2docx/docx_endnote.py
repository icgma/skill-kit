...
def process_word_document_with_endnotes(
    ...
    try:
        ...
        update_document_rels(temp_path, nsmap, 'endnote')
        ...
        if sect_prs:
            ...
            if last_sect_pr.find('w:endnotePr', nsmap) is None:
                ...
                # CT_SectPr order: headerReference/footerReference, footnotePr,
                # endnotePr, type, pgSz, ... — insert at the legal slot.
                footnote_pr = last_sect_pr.find('w:footnotePr', nsmap)
                if footnote_pr is not None:
                    footnote_pr.addnext(endnote_pr)
                else:
                    anchor = last_sect_pr.find('w:type', nsmap)
                    if anchor is None:
                        anchor = last_sect_pr.find('w:pgSz', nsmap)
                    if anchor is not None:
                        anchor.addprevious(endnote_pr)
                    else:
                        last_sect_pr.append(endnote_pr)

        tree.write(str(doc_xml_path), encoding='utf-8', xml_declaration=True, standalone=True)
        update_content_types(temp_path, nsmap, 'endnote')
...
def _create_endnotes(citation_numbers, citation_db: dict, nsmap):
    ...
    # (numbering format lives in sectPr/endnotePr — w:endnotes allows only w:endnote children)
