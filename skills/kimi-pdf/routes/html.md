...
## Step 0: Requirements
Requires Node.js, Playwright, and Chromium already present in the environment.
The conversion script resolves the browser across macOS/Linux/Windows
automatically. Both missing-dependency cases fail with **exit code 2** and a
guidance message on stderr — never a stack trace — and never auto-install
anything:

- **Playwright package not resolvable** → "Playwright module not found" (exit 2)
- **Chromium binary not found** (Playwright present, no fallback browser) → "Chromium browser not found" (exit 2)

On exit 2, fall back to `routes/reportlab.md` — do not install the browser
stack unless the user explicitly wants this route.
...
```bash
node "{skill_path}/scripts/html_to_pdf.js" document.html --output output.pdf
```
...
Use `target-counter()` for TOC page numbers:
```css
.toc a::after {
    content: target-counter(attr(href url), page);
    float: right;
}
/* Dotted leader line via border — NOT leader(), see warning below */
.toc li { border-bottom: 1px dotted #999; }
```
TOC items **must** be `<a href="#section-id">`, not plain text.

**WARNING**: the bundled Paged.js polyfill does not implement `leader()`.
`content: leader('.') target-counter(...)` is discarded as a whole — the page
number disappears along with the dots. Never use `leader()`; draw dot leaders
with borders or backgrounds instead.
...
For Markdown-first research reports, source markers such as `[^123^]` map by exact numeric `id` to the citation JSONL supplied with the Markdown; if none is supplied but `/mnt/agents/.store/citation.jsonl` exists, use it. Treat the Markdown as canonical content and the HTML as a presentation-only projection. Use the matched metadata to build whatever citation treatment fits the PDF. References are optional rather than mandatory for this handoff. If an ID cannot be resolved, preserve the marker and report it instead of inventing or dropping a source.
...
```
```css
/* TOC item: auto page number (never leader() — see Advanced TOC warning) */
.toc a::after {
    content: target-counter(attr(href url), page);
    float: right;
}
...
### CJK Fonts
The environment needs CJK fonts installed, or use web fonts:
```html
...
```
**Note**: The bundled Paged.js does not implement `leader('.')` at all (see Advanced TOC warning). For RTL documents, build the TOC with a table layout instead.
