...
# and CJK markdown content in log/stdout lines would crash the print.
...
def find_cjk_font(explicit: str = None) -> str:
    """Locate a CJK-capable font file. Order: explicit arg, env, runtime, candidates, fc-list."""
    # Managed-runtime bundled font, derived from the interpreter layout
    # (<runtime>/python/.venv → <runtime>/python/fonts) so it works even when
    # the DAIMON_CJK_FONT_REGULAR env var is not set. Exists-check gated;
    # harmless under any other Python.
    runtime_font = Path(sys.prefix).parent / "fonts" / "NotoSansSC-Regular.ttf"
    for cand in (
        explicit,
...
def _build_css(font_path: str) -> str:
    ...
    CJK strategy (picked by caller, encoded in font_path):
    - ""                    → built-in CID font STSong-Light, which xhtml2pdf
                              auto-registers via its asianFontList — no font
                              file needed, so CJK can never tofu. Glyphs are
                              non-embedded (viewer-substituted).
    - .ttf/.otf file        → @font-face embed (reliable in xhtml2pdf)
    - .ttc collection       → NOT used: the @font-face loader calls
                              TTFont(name, file) without subfontIndex and
                              silently produces tofu; the caller maps .ttc
                              finds to the CID fallback instead.
    """
    font_face = ''
    if font_path:
        ...
        body_family = '"STSong-Light", sans-serif'
    # When a CJK font is in play, use it for code too so CJK comments inside
    ...
    code_family = body_family.replace('sans-serif', 'monospace')
    return f"""
{font_face}
...
def convert_md_to_pdf(md_path: str, output_path: str = None, font: str = None,
    ...
    # 4. Wrap with CSS (CJK font: .ttf/.otf embed → built-in CID fallback)
    font_path = find_cjk_font(font)
    if font_path and font_path.lower().endswith('.ttc'):
        logger.info(f"CJK font {font_path} is a .ttc collection — the "
                    "xhtml2pdf @font-face loader cannot pick a subfont; "
                    "using built-in CID font instead")
        font_path = ''
    if font_path:
        logger.info(f"CJK font: {font_path}")
    else:
        logger.info("CJK font: built-in STSong-Light (CID, non-embedded)")
    full_html = (
...
if __name__ == '__main__':
    ...
    parser.add_argument('--references-heading', default='References',
                        help='Heading for the consolidated references section (e.g. References)')
