...
// Playwright may be absent in minimal/offline environments. Degrade cleanly
// (guidance + exit 2) instead of crashing at module scope — see
// routes/html.md Step 0. Never auto-install here; the caller picks a route.
let chromium;
try {
  ({ chromium } = loadPlaywright());
} catch (error) {
  console.error('\n❌ Playwright module not found.');
  console.error('   This route needs Node.js + Playwright + Chromium already installed.');
  console.error('   To enable it:');
  console.error('     npm install playwright');
  console.error('     npx playwright install chromium');
  console.error('   Or point PLAYWRIGHT_PATH at an existing Playwright install.');
  console.error('   When only the Chromium binary is missing, an installed system');
  console.error('   Chrome/Edge/Chromium is used as fallback (override: PLAYWRIGHT_CHROMIUM_PATH).');
  console.error('   Otherwise fall back to the ReportLab route (routes/reportlab.md).');
  console.error('');
  process.exit(2);
}
...
async function convertHTMLToPDF(inputFile, outputFile, customCSS) {
  ...
  // Resolve an existing browser only; never auto-install (the caller decides
  // whether this route is viable — see routes/html.md Step 0).
  const browserInfo = resolveChromium({ chromium });
  ...
  try {
    ...
    // Load HTML content (pathToFileURL handles Windows drive letters/spaces)
