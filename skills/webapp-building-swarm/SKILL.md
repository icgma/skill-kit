...
Build in a **local worktree**, never on the shared coordination repo. For final delivery, follow `vibecoding-webapp-swarm` Phase 7 step 4: reinstall dependencies after all merges, then run the production build before `build_version`. Do not copy `dist/` by hand; the orchestration skill defines the static `project_dir` and version flow.
